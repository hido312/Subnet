...
class BaseModel(nn.Module):
    ...
    def _reduce_complexity_separately(sub_band_input, full_band_output, device):
        ...
        Notes:
            ...
            2. batch size는 3의 배수여야 ㄴ함. 그렇지 않으면 마지막 일부 배치 내 주파수가 잘 훈련되지 않음
    ...
    def sband_forgetting_norm(input, train_sample_length):
        """
        Forgetting Norm와 유사하지만, 결합된 모델의 중간 주파수 대역을 사용하여 평균을 계산함.
        하지만, 효과가 좋지 않음
    ...
    def forgetting_norm(input, sample_length_in_training):
        """
        입력은 3차원이며, 현재 norm에 대한 평균으로서 계속해서 주변의 평균을 추정

        Args:
            input: [B, F, T]
            sample_length_in_training: 훈련 중에 사용되는 길이는 equalization 계수를 계산하는 데 사용
