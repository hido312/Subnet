...
class BaseInferencer:
    ...
    def _unfold(input, pad_mode, n_neighbor):
        """
        주파수 축을 따라 음성 스펙트로그램을 겹쳐진 여러 하위 주파수 대역으로 분할

        Args:
            input: [B, C, F, T]

        Returns:
            [B, N, C, F, T], 이때 F : subnet 주파수 축 개수
            e.g. [2, 161, 1, 19, 200]
        """
        ...
        # unfold 중간차원 분리
        output = output.reshape(batch_size, n_channels, sub_band_n_freqs, n_frames, n_freqs) # reshape : 데이터 순서 유지, 배열 모양 변경 (원소의 순서는 메모리 저장 순서대로 유지됨)
        output = output.permute(0, 4, 1, 2, 3).contiguous()                                  # permute : 데이터 순서 변경하여 축을 바꿈   (contiguous 이용하면 메모리 저장된 순서 유지함)
        return output
